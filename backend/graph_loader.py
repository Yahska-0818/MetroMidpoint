    ...
    def build_or_load_graph(self) -> nx.Graph:
        ...
        for line, group in df.groupby("Line"):
            ...
            for i in range(len(nodes) - 1):
                ...
                G.add_edge(u["Node Name"], v["Node Name"], weight=weight, distance=physical_distance, type="travel")
        for station, group in df.groupby("Station Name"):
            ...
            for i in range(len(nodes)):
                for j in range(i + 1, len(nodes)):
                    G.add_edge(nodes[i], nodes[j], weight=penalty, distance=0.0, type="interchange")
