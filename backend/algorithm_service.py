...
class AlgorithmService:
    def __init__(self, graph: nx.Graph, csv_url: str):
        ...
        self.df["Station Name"] = self.df["Station Name"].str.replace(r"\s*\[.*\]", "", regex=True).str.strip()
    ...
    def _precompute_all_paths(self):
        for node in self.graph.nodes:
            ...
            d_int, p_int = nx.single_source_dijkstra(self.graph, node, weight="interchange_cost")
            self.all_pairs_fewest_interchanges[node] = {"distances": d_int, "paths": p_int}
    ...
    def _resolve_station_name(self, station_name: str) -> str:
        ...
        possible = self.df[self.df["Station Name"].str.lower() == station_name.lower()]["Node Name"].tolist()
        if not possible:
    ...
    def get_route_details_least_interchanges(self, source: str, destination: str) -> Dict[str, Any]:
        return self._build_route_response(source, destination, self.all_pairs_fewest_interchanges)

    def _build_route_response(self, source: str, destination: str, target_dict: dict) -> Dict[str, Any]:
        ...
        time = sum(self.graph[path[i]][path[i + 1]].get("time", 3.0) for i in range(len(path) - 1))
        interchanges = sum(1 for i in range(len(path) - 1) if self.graph[path[i]][path[i + 1]]["type"] == "interchange")
        formatted_path = []
        for node in path:
    ...
    def find_best_midpoint(self, start_stations: List[str]) -> Dict[str, Any]:
        ...
        valid_nodes = []
        for node in self.graph.nodes:
            if all(node in self.all_pairs_shortest[s]["distances"] for s in resolved_start_nodes):
                valid_nodes.append(node)
        if not valid_nodes:
            raise ValueError("No path exists between all participants")
        candidates = []
        for node in valid_nodes:
            node_times = [self.all_pairs_shortest[s]["distances"][node] for s in resolved_start_nodes]
            total_interchanges = 0
            for s in resolved_start_nodes:
                p = self.all_pairs_shortest[s]["paths"][node]
                total_interchanges += sum(1 for i in range(len(p) - 1) if self.graph[p[i]][p[i + 1]]["type"] == "interchange")
            candidates.append({
                "node": node,
        ...
        best = sorted(candidates, key=lambda x: (x["max_time"], x["total_time"], -x["centrality"], x["total_interchanges"]))[0]
